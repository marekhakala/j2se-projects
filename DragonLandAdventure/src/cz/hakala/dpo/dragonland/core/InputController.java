...
import cz.hakala.dpo.dragonland.api.InputControllerInterface;

public class InputController implements InputControllerInterface {
	private GameViewInterface view = null;

	public InputController(GameViewInterface view) {
		this.view = view;
	}

	public void readInputLine() throws IOException {
		BufferedReader in = new BufferedReader(new InputStreamReader(System.in));
		String s;

		while ((s = in.readLine()) != null) {
			String[] params = s.split(" ");
			CommandInterface command = GameApplication.getInstance()
					.getCommand(params[0]);

			if (command != null)
				view.printText(command.execute(params) + "\n");
		}
